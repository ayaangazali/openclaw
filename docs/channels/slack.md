...
- The `channels.slack.socketMode` object, including `clientPingTimeout`, `serverPingTimeout`, and `pingPongLoggingEnabled`, is retired. Slack config validation rejects it at both `channels.slack.socketMode` and `channels.slack.accounts.<accountId>.socketMode`, so a config that still sets it fails to load. Remove the key.
